...
#some static metadata can be added here like env ot type
my $fmt = <<"END_MSG";
REGEX AllRecords
(.*)
hostname LABEL
...
while(<LOGFILE_LIST>) {
	...
	###################################################################################################
	# Useful information that inform about source of log entry can be extracted from logfile path
	# like instance name or cluster name
	...
	push @files, "${file}_xxx.conf" if $opt_t;		#add files to array that will be used by tar function
	open(CONF,">${file}_xxx.conf") or die "Cannot open file: $!";
	...
	print CONF "BufEvtPath=/apps/scala/logs/${file}_xxx.cache\012";
	$conf =~ s/\015//g;			                    #strip windows CR if script is executed on windows
	print CONF $conf;
	close CONF;
	
	push @files, "${file}_xxx.fmt" if $opt_t;
	open(FMT,">${file}_xxx.fmt") or die "Cannot open file: $!";;
	binmode FMT;
	$fmt =~ s/\015//g;
	print FMT $fmt;
	
	####################################################################################################
	# Metadata dynamically extracted form log path can be added here
