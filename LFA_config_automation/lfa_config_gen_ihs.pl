...
while(<LOGFILE_LIST>) {
	...
	push @files, "${file}_ihs.conf" if $opt_t;		#add files to array that will be used by tar function
	open(CONF,">${file}_ihs.conf") or die "Cannot open file: $!";
	...
	print CONF "BufEvtPath=/apps/scala/logs/${file}_ihs.cache\012";
	$conf =~ s/\015//g;			                    #strip windows CR if script is executed on windows
	print CONF $conf;
	close CONF;
	
	push @files, "${file}_ihs.fmt" if $opt_t;
	open(FMT,">${file}_ihs.fmt") or die "Cannot open file: $!";;
	binmode FMT;
	$fmt =~ s/\015//g;
	print FMT $fmt;
	print FMT "cluster NONE\012";
	if ($file =~ /access_log/) {
		print FMT "module access.log\012";
	} else {
		print FMT "module error.log\012";
	}
